...
def ideal_mixphase_stft(track, device):
    ...
    for name, source in track.sources.items():
        ...
        Yj = phasemix_sep(torch.view_as_real(X), source_mag)

        # set this as the source estimate
        estimates[name] = stft_bwd(torch.view_as_complex(Yj), N)

    # residual = other
    other_mag = Xmag - (P['bass'] + P['drums'] + P['vocals'])

    Y_other = phasemix_sep(torch.view_as_real(X), other_mag)
    estimates['other'] = stft_bwd(torch.view_as_complex(Y_other), N)
